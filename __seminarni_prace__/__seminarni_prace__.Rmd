...
\textbf{Klíčová slova:} tržní cena nemovitosti, odhad ceny nemovitosti, Zestimate, Zillow, kaggle.com, panelová regrese, krátký panel, model se smíšenými efekty, fixní efekty, náhodné efekty, predikce, střední absolutní chyba, strojové učení, machine learning
...
\subsection{Přehled vysvětlujících proměnných}

Zde je uveden přehled všech vysvětlujících proměnných dostupných v datastech nabízených soutěží na kaggle.com.

\bigskip

\begin{labeling}{calculatedfinishedsquarefeet \quad}

  \item[airconditioningtypeid] Typ chladícího zařízení, kterým je nemovitost vybavena.
  \item[architecturalstyletypeid] Architektonický styl nemovitosti (sloh).
  \item[basementsqft] Obyvatelná plocha pod úrovní země [čtvereční stopy].
  \item[bathroomcnt] Počet koupelen v nemovitosti.
  \item[bedroomcnt] Počet ložnic v nemovitosti
  \item[buildingqualitytypeid] Celkový odhad nemovitosti na škále špatná--dobrá.
  \item[buildingclasstypeid] Materiál základů stavby (beton, ocel, dřevo apod.).
  \item[calculatedbathnbr] Standardizovaný počet koupelen v nemovitosti (nemusí být celé číslo).
  \item[decktypeid] Typ vyšších podlaží (pater, jsou-li přítomna).
  \item[threequarterbathnbr] Počet "tříčtvrtečních koupelen" v nemovitosti (sprchový kout \& dřez \& toaleta).
  \item[finishedfloor1squarefeet] Obytná plocha na prvním patře nemovitosti [čtvereční stopy].
  \item[calculatedfinishedsquarefeet] Odhadnutá celková obytná plocha nemovitosti [čtvereční stopy].
  \item[finishedsquarefeet6] Obytná i neobytná plocha přízemí [čtvereční stopy].
  \item[finishedsquarefeet12] Dostavená obytná plocha [čtvereční stopy].
  \item[finishedsquarefeet13] Okrajová (?) obytná plocha [čtvereční stopy].
  \item[finishedsquarefeet15] Celková plocha nemovitosti [čtvereční stopy].
  \item[finishedsquarefeet50] Obytná plocha v patře, do kterého vedou vstupní dveře [čtvereční stopy].
  \item[fips] Federal Information Processing Standard code.
  \item[fireplacecnt] Počet krbů v nemovitosti.
  \item[fireplaceflag] Je alespoň jeden krb v domě?
  \item[fullbathcnt] Počet "kompletních" koupelen v nemovitosti (sprchový kout \& vana \& dřez \& toaleta).
  \item[garagecarcnt] Celkový počet garáží.
  \item[garagetotalsqft] Celková plocha garážování [čtvereční stopy].
  \item[hashottuborspa] Má nemovitost lázně nebo saunu?
  \item[heatingorsystemtypeid] Typ ohřevného systému instalovaného v nemovitosti.
  \item[latitude] Zeměpisná šířka geometrického středu nemovitosti $\times 10^6$.
  \item[longitude] Zeměpisná délka geometrického středu nemovitosti $\times 10^6$.
  \item[lotsizesquarefeet] Plocha parcely [čtvereční stopy].
  \item[numberofstories] Počet podlaží nemovitosti.
  \item[parcelid] Jednoznačný identifikátor nemovitosti.
  \item[poolcnt] Počet bazénů na parcele.
  \item[poolsizesum] Celková plocha dostupných bazénů na parcele [čtvereční stopy].
  \item[pooltypeid10] Dummy proměnná pro přítomnost lázní nebo sauny v nemovitosti.
  \item[pooltypeid2] Dummy proměnná pro přítomnost bazénu s lázněmi nebo saunou v nemovitosti.
  \item[pooltypeid7] Dummy proměnná pro přítomnost bazénu bez lázní a sauny v nemovitosti.
  \item[propertycountylandusecode] Kód oblasti.
  \item[propertylandusetypeid] Typ okolí.
  \item[propertyzoningdesc] Description of the allowed land uses (zoning) for that property.
  \item[rawcensustractandblock] Kombinace traktu censu a blokového ID (?).
  \item[censustractandblock] Kombinace traktu censu a blokového ID (?).
  \item[regionidcounty] Typ oblasti, ve které je nemovitost situována.
  \item[regionidcity] Město, kde je nemovitost lokalizována (pokud je ve městě).
  \item[regionidzip] Poštovní směrovací číslo oblasti, kde se nemovitost nachází.
  \item[regionidneighborhood] Sousedství, ve kterém se nemovitost nachází.
  \item[roomcnt] Celkový počet místností v nemovitosti.
  \item[storytypeid] Typ podlaží ve velkokapacitní nemovitosti.
  \item[typeconstructiontypeid] Materiál použitý ke konstrukci nemovitosti.
  \item[unitcnt] Počet jednotek, ze kterých se nemovitost skládá.
  \item[yardbuildingsqft17] Plocha terasy [čtvereční stopy].
  \item[yardbuildingsqft26] Plocha zahradního domku [čtvereční stopy].
  \item[yearbuilt] Rok dostavění nemovitosti.
  \item[taxvaluedollarcnt] Celková odhadnutá cena parcely.
  \item[structuretaxvaluedollarcnt] Celková odhadnutá cena nemovitosti (bez parcely).
  \item[landtaxvaluedollarcnt] Celková odhadnutá cena půdy, kterou parcela zabírá.
  \item[taxamount] Celkový odhad bohatství rovnajícího se ročnímu zisku \mbox{z nemovitosti}, parcely a půdy.
  \item[assessmentyear] Skutečná hodnota bohatství rovnajícího se ročnímu zisku \mbox{z nemovitosti}, parcely a půdy.
  \item[taxdelinquencyflag] Daně z nemovitosti byly splaceny do konce roku 2005.
  \item[taxdelinquencyyear] Rok, kdy mají být splaceny všechny nedoplatky z nemovitosti.
...
\subsubsection{\texttt{data\_loading.R}}
...
\subsubsection{\texttt{data\_processing.R}}
...
\subsubsection{\texttt{data\_saving\_and\_reloading.R}}
...
\subsubsection{\texttt{data\_postprocessing.R}}
...
\subsubsection{\texttt{exploratory\_data\_analysis.R}}
...
\subsubsection{\texttt{hierarchical\_model.R}}
