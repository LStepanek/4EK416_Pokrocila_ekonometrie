...
Tato práce předkládá pokus o vytvoření (nového) modelu odhadu ceny nemovitostí v podobném smyslu, v jakém je již jedenáct let odhaduje pro americký trh společnost Zillow [@Zestimate]. Současně jde o řešení seminárního zadání v rámci předmětu 4EK416 Pokročilá ekonometrie, vyučovaném na Vysoké škole ekonomické v Praze. V neposlední řadě práce předkládá i možné řešení veřejně dostupné soutěže portálu kaggle.com, aktuálně jednoho z nejznámějších webů v oblasti \textit{data science}.
...
kde $\bm{y}$ je vektor závisle proměnné (v našem případě \texttt{logerror}) tak, že platí $\mathbf{E}(\bm{y}) = \bm{X} \bm{\beta}$; $\bm{X}$ je designová matice fixních efektů (sem patří prakticky všechny uvažované regresory), $\bm{\beta}$ je matice koeficientů fixních efektů, $\bm{u}$ je vektor koeficientů náhodných efektů, též někdy nepřesně nazývaný \textit{grouping vector} tak, že platí $\mathbf{E}(\bm{u}) = \bm{0}$ a $\mathrm{var}(\bm{u}) = \bm{G}$; $\bm{Z}$ je designová matice náhodných efektů (v našem případě sem patří primární klíč \texttt{parcelid} identifikující nemovitosti v datasetu s možnými opakovanými prodeji některých nemovitostí), $\bm{\varepsilon}$ je vektor náhodných chyb tak, že platí $\mathbf{E}(\bm{\varepsilon}) = \bm{0}$ a $\mathrm{var}(\bm{\varepsilon}) = \bm{R}$.
...
V případě kategorických proměnných dle korelogramu \ref{korelogram_factor} vidíme celkem zřejmě, že proměnné s indexy 3, 4, 5, 9, 13, 14 a 16 spolu prakticky funkčně (absolutně) korelují. Ze této skupiny ponechme pro účely modelu smíšených efektů jen proměnnou s indexem 3 (\texttt{airconditioningtypeid}).
